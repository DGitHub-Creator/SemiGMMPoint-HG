...
from openpoints.utils.finite_check import assert_finite
...
class GMMSegHead(nn.Module):
    ...
    def __init__(self, 
        ...
        self.IGNIRE_INDEX = ignore_index
    ...
    def forward_help(self, base_feature, gt_semantic_seg=None, train_cfg=None, test_cfg=None):
        ...
        _c = l2_normalize(_c)
        assert_finite(_c, 'GMMHead _c (feat_norm + l2)', epoch=getattr(self, '_epoch', None),
                      iteration=getattr(self, '_iteration', None), rank=getattr(self, '_rank', None))

        self.means.data.copy_(l2_normalize(self.means))
        assert_finite(self.means, 'GMMHead means', epoch=getattr(self, '_epoch', None),
                      iteration=getattr(self, '_iteration', None), rank=getattr(self, '_rank', None))
        assert_finite(self.diagonal, 'GMMHead diagonal', epoch=getattr(self, '_epoch', None),
                      iteration=getattr(self, '_iteration', None), rank=getattr(self, '_rank', None))
        #
    ...
    def compute_log_prob(self, _fea):
        assert_finite(_fea, 'GMMHead compute_log_prob _fea', epoch=getattr(self, '_epoch', None),
                      iteration=getattr(self, '_iteration', None), rank=getattr(self, '_rank', None))
        covariances = self.diagonal.detach_() # * c,p,d,d
        assert_finite(covariances, 'GMMHead diagonal in compute_log_prob',
    ...
    def update_GMM(self, unique_c_list):
        ...
        for _c in unique_c_list:
            ...
            for _p in range(self.num_components):
                ...
                # Skip components whose memory is empty or all-zero: no reliable
                # statistics can be estimated, keep the previous mean/covariance.
                _mem_sum = _mem_fea_q.abs().sum(dim=0).sum()
                if _mem_sum <= 0:
                    continue
        ...
        # The GMM state is shared across DDP ranks: rank-local queues would make
        # the two ranks diverge, so after each EM update the mean/diagonal are
        # averaged over all ranks (only when a process group is active).
        if torch.distributed.is_available() and torch.distributed.is_initialized() \
                and torch.distributed.get_world_size() > 1:
            torch.distributed.all_reduce(self.means, op=torch.distributed.ReduceOp.SUM)
            self.means /= torch.distributed.get_world_size()
            torch.distributed.all_reduce(self.diagonal, op=torch.distributed.ReduceOp.SUM)
            self.diagonal /= torch.distributed.get_world_size()
        self.means.data.copy_(l2_normalize(self.means))
