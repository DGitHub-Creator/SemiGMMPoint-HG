...
class BaseSeg(nn.Module):
    ...
    def forward(self, data):
        ...
        if self.decoder is not None:
            f = self.decoder(p, f).squeeze(-1)   # assanet:  torch.Size([8, 64, 36000])
        if self.head is not None:
            f = self.head(f)
        return f

@MODELS.register_module()
class GMMBaseSeg(nn.Module):
    def __init__(self,
                 encoder_args=None,
                 decoder_args=None,
                 cls_args=None,
    ...
    def forward(self, data):
        ...
        if self.head is not None:
            if data.get('y',None) is not None:
                self.train_cfg['eval'] = False
                losses,f,feat = self.head(f,data['y'],self.train_cfg)
                return f,feat
            else:
                self.train_cfg['eval'] = True
                losses,f,feat = self.head(f,None,self.train_cfg)
                return f,feat
        return f


@MODELS.register_module()
class PreGMMBaseSeg(nn.Module):
    def __init__(self,
                 encoder_args=None,
                 decoder_args=None,
                 cls_args=None,
...
@MODELS.register_module()
class SemiGMMBaseSeg(nn.Module):
    def __init__(self,
                 encoder_args=None,
                 decoder_args=None,
                 cls_args=None,
