...
class ChromaticAutoContrast(object):
    ...
    def __call__(self, data):
        if np.random.rand() < self.p:
            ...
            # A constant channel (hi == lo) would make 255 / (hi - lo) == inf and
            # produce NaN colors. Constant channels keep their value (scale = 0).
            rg = hi - lo
            scale = np.zeros_like(rg, dtype=np.float32)
            nz = rg > 0
            scale[nz] = 255.0 / rg[nz]
