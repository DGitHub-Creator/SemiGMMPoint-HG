...
def load_gmm_checkpoint(model, pretrained_path, module=None):
    if not os.path.exists(pretrained_path):
...
def load_checkpoint(model, pretrained_path, module=None):
    ...
    base_ckpt_tmp = dict()
    if module is not None:
        for module_item in module.split(','):
            module_item = module_item.strip()
            tmp = {k:v for k, v in base_ckpt.items() if module_item in k}
            base_ckpt_tmp.update(**tmp)
    if module is not None:
        base_ckpt = base_ckpt_tmp
    if "bert" in list(ckpt_state_dict.items())[0][0]:
        base_ckpt=bert2vit_ckpt_rename(ckpt_state_dict)
        #state_dict has "qkv.value" key, will be mis-regonized as metric and over flush the command ouput
        state_dict=base_ckpt
    ...
    if hasattr(model, 'module'):
        ...
        incompatible = model.load_state_dict(base_ckpt, strict=False)
    logging.info('missing_keys=%d: %s'
                 % (len(incompatible.missing_keys), incompatible.missing_keys))
    logging.info('unexpected_keys=%d: %s'
                 % (len(incompatible.unexpected_keys), incompatible.unexpected_keys))
    loaded = len(base_ckpt) - len(incompatible.unexpected_keys)
    logging.info('Successful Loading the ckpt from %s (loaded %d/%d ckpt keys)'
                 % (pretrained_path, loaded, len(base_ckpt)))
