...
matrix (rows = GT, columns = Prediction). Metrics:

    IoU_c = TP_c / (TP_c + FP_c + FN_c)
    Acc_c = TP_c / GT_c
    mIoU  = mean of the 6 class IoUs
    mAcc  = mean of the 6 class Accs
    OA    = correct points / valid test points
...
Outputs (written to --outdir):
    metrics.json / metrics.csv / metrics.txt / confusion_matrix.csv
"""
import argparse
import csv
import json
...
BUDGET_RATIO = {10: 0.1431, 20: 0.2117}
BUDGET_SCENES = {10: 2, 20: 3}


def load_scene(path, cfg):
    """Replicates examples/segmentation/*_main.py::load_data for odpt."""
    ...
    label = np.asarray(label).reshape(-1).astype(np.int)
    coord -= coord.min(0)
    idx_points = []
    voxel_idx = reverse_idx_part = reverse_idx_sort = None
...
def predict_scene(model, data_path, cfg, pipe_transform, gravity_dim):
    ...
    return pred, label.cpu().numpy() if label is not None else None
...
def metrics_from_cm(cm):
    ...
    macc = float(accs.mean())
    return oa, macc, miou, ious, accs
...
def main():
    ...
    assert cfg.dataset.common.test_scenes == TEST_SCENES, \
        f'test scenes mismatch: {cfg.dataset.common.test_scenes}'
    ...
    for scene in TEST_SCENES:
        ...
        assert len(pred) == len(label), \
            f'{scene}: pred len {len(pred)} != gt len {len(label)}'
        assert pred.min() >= 0 and pred.max() <= cfg.num_classes - 1, \
            f'{scene}: prediction values outside 0..{cfg.num_classes - 1}'
        valid = label != 255
        if not valid.all():
        ...
        assert lab.min() >= 0 and lab.max() <= cfg.num_classes - 1, \
            f'{scene}: GT values outside 0..{cfg.num_classes - 1}'
        for c in range(cfg.num_classes):
            global_cm[c, :] += np.bincount(pred_v[lab == c], minlength=cfg.num_classes)
        per_scene[scene] = {
            'valid_points': int(len(lab)),
    ...
    assert global_cm.sum() == total_valid, \
        f'confusion matrix total {global_cm.sum()} != valid points {total_valid}'
    ...
    result = {
        ...
        'labeled_scenes': BUDGET_SCENES[budget],
        'labeled_point_ratio': BUDGET_RATIO[budget],
        'test_area': 'Area_3',
    ...
    with open(os.path.join(args.outdir, 'metrics.txt'), 'w') as f:
        f.write(f'SemiGMMPoint {budget}% (Area_3, {global_cm.sum()} valid points)\n')
        ...
        f.write('       ' + ' '.join(f'{c:>10d}' for c in range(6)) + '\n')
        for i in range(6):
    ...
    with open(os.path.join(args.outdir, 'metrics.csv'), 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(['method', 'budget_alias', 'labeled_scenes', 'labeled_point_ratio',
                    'test_area', 'checkpoint', 'epoch', 'seed', 'OA', 'mAcc', 'mIoU'])
        w.writerow(['SemiGMMPoint', f'{budget}%', BUDGET_SCENES[budget], BUDGET_RATIO[budget],
                    'Area_3', os.path.abspath(args.checkpoint), int(ckpt_epoch), int(cfg.seed),
